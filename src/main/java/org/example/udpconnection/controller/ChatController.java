	...
	public void handleEstablishConnection(ActionEvent actionEvent) {
		try {
			...
			if (this.connection.isConnectionOpen()) {
				...
				appendMessage("Connection closed in port " +
						this.connection.getDestinationPort() + ".\n");
			}
			connection.setPorts(ip, srcPort, destPort);
			connection.startReceiving();
			messageArea.appendText("Connection successfully established by port " +
					destPort + ".\n");
		} catch (Exception e) {
			messageArea.appendText("Establish connection failed: " +
					e.getMessage() + "\n");
